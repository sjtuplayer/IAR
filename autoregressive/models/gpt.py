...
class Attention(nn.Module):
    ...
    def forward(
        ...
        bsz, seqlen, _ = x.shape  # x:32*576*768
...
class Transformer(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        # self.sub_vocab_size = config.cluster_size
        self.sub_vocab_size = config.cluster_size
        self.cluster_num = config.vocab_size // self.sub_vocab_size
    ...
    def forward(
            self,
            idx: torch.Tensor,  # shape:[32,575]
            cond_idx: torch.Tensor,  # cond_idx_or_embed   shape:[32]
            input_pos: Optional[torch.Tensor] = None,
            targets: Optional[torch.Tensor] = None,  # shape:[32,576]
            mask: Optional[torch.Tensor] = None,
        ...
        # output layers
        h = self.norm(h)
        logits = self.output(h).float()  # b*t*16384
        logits1=logits.view(logits.size(0), logits.size(1), -1, self.sub_vocab_size)# b*t*128*128
        alpha=1
        logits1=(torch.logsumexp(alpha * logits1, dim=-1) / alpha)
        if self.training:
        ...
        if targets is not None:
            targets1, targets2 = targets // self.sub_vocab_size, targets % self.sub_vocab_size
            loss1 = F.cross_entropy(logits1.view(-1, logits1.size(-1)), targets1.view(-1))
            loss2 = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1))
